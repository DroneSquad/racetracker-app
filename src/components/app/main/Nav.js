...
export default class Nav extends React.Component {
  constructor(props) {
    ...
    this.NavItems = _.map(ROUTES, (nav, id) => {
      ...
      nav.id = id;
      if (window.location.href.indexOf(nav.path) > 0) {
        // set the current index when first loading to the page
        currentPageIndex = id;
      }
      return (
        <BottomNavigationItem
          key={id}
          label={nav.name}
          icon={<FontIcon className={nav.iconClasses} />}
          onTouchTap={() => this.onClick(nav)}
        />
      );
    });
  ...
  onHistory(event) {
    ...
    if (event.pathname === '/404') {
      ...
      this.routeIndex.forEach((path, id) => {
        ...
        if (index > -1) {
          // find which page we are on and update the nav button selected
