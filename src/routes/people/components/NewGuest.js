...
export default class NewGuest extends React.Component {
  ...
  render() {
    ...
    let saveBtnClass = this.state.saving
      ? 'ds-gray-alt-text mdi mdi-loading spinner'
      : 'ds-gray-alt-text mdi mdi-check';
    return (
      <div className="main white-bg">
        <header>
          <AppBar
            ...
            iconClassNameRight={saveBtnClass}
            onRightIconButtonTouchTap={this.onSave}
          />
        </header>
        <main>
          <form>
            <List>
              <ListItem
                disabled
                leftIcon={<FontIcon className="mdi mdi-account" />}
                innerDivStyle={{ padding: '0 0 0 48px' }}
              >
                <TextField
                  value={this.state.callsign}
                  onChange={this.onCallsign}
                  hintText="Callsign*"
                  errorText={!this.state.callsign && this.state.enteredCallsign && 'Callsign is required'}
                />
              </ListItem>
              ...
              <ListItem
                disabled
                leftIcon={<FontIcon className="mdi mdi-email" />}
                innerDivStyle={{ padding: '0 0 0 48px' }}
              >
                <TextField
                  ...
                  hintText="Email*"
                  errorText={!this.state.email && this.state.enteredEmail && 'Email is required'}
                />
              </ListItem>
              <ListItem disabled>
