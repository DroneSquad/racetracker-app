...
export default class Group extends React.Component {
  ...
  members() {
    ...
    return (
      <ListItem id={id} className="group-item recent-pilots" leftIcon={icon} onTouchTap={() => this.props.showMembers(this.id)}>
  ...
  groupActions() {
    return _.map(this.props.actions, (action, key) => (
      <BottomNavigationItem
        key={key}
        label={<span className="ds-blue-text">{action.name}</span>}
  ...
  render() {
    ...
    let { name, createdById, loading, memberCount } = this.props.group;
    return (
      <List className="group-details">
        {/* The banner / navbar */}
        <ListItem className="group-item banner">
          <span className={loading && 'loading-bar'}>
            <img className="bar-item" height={175} src={api.urls.banner(this.id)} alt="" style={{ width: '100%' }} />
          </span>
        </ListItem>
        <Divider />
        ...
        <ListItem className="group-item ds-white-text ds-blue info">
          <div className={loading && 'loading-bar'}>
            <div className="bar-item group-name">{name}</div>
            <div className="bar-item group-info">We are a group of {memberCount || 1} {pluritize('pilot', memberCount)} in {formatAddress(this.props.group)}</div>
            <div className="bar-item group-admins">Organised by {createdById}</div>
          </div>
        </ListItem>
        <Divider />
